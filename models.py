...
class TReS(object):
	...
	def __init__(self, config, device,  svPath, datapath, train_idx, test_idx, Net):
		...
		self.k = self.lr/53
		# self.solver = torch.optim.Adam(self.paras, weight_decay=self.weight_decay, betas=(1-self.k*(1-0.9), 1-self.k*(1-0.999)))
	...
	def train(self,seed,svPath):
		best_srcc = 0.0
		best_plcc = 0.0
		print('Epoch\tTrain_Loss\tTrain_SRCC\tVal_SRCC\tVal_PLCC\tLearning_Rate\tdroplr')
		steps = 0
		train_results = {}
		results = {}
